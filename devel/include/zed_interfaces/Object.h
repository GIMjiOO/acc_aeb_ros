  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::zed_interfaces::Object_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.position_covariance.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.position_covariance[i]);
    }
    if (v.position_covariance.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "dimensions_3d: ";
    if (v.dimensions_3d.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.dimensions_3d.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.dimensions_3d[i]);
    }
    if (v.dimensions_3d.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.head_position.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.head_position[i]);
    }
    if (v.head_position.empty() || true)
      s << "]";
    if (true || !indent.empty())
